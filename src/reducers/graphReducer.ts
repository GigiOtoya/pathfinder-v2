...
export const graphReducer = (state: GraphState, action: GraphAction) => {
  switch (action.type) {
    case "NEW_GRAPH":
      const newGraph = new Graph(action.graph, state.graph.weighted);
      return {
        ...state,
        graph: newGraph,
      };
    ...
    case "DRAG_VERTEX":
      if (state.active) {
        const newGraph = new Graph(state.graph, state.graph.weighted);
    ...
    case "WEIGHTED": {
      const newGraph = new Graph(state.graph, true, state.graph.directed, true);
    ...
    case "UNWEIGHTED": {
      const newGraph = new Graph(state.graph, false, state.graph.directed, true);
